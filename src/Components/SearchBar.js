...
export const SearchBar = () => {
  ...
  const results = GETCARDS.filter((card) => {
    return (
      searchTerm &&
      card &&
      card.genre &&
      card.genre.toLowerCase().includes(searchTerm)
    );
  });
    ...
    <>
      <div class="m-5">
        <form class="max-w-md mx-auto">
          <label
            for="default-search"
            class="mb-2 text-sm font-medium text-gray-900 sr-only text-black"
          >
            Search
          </label>
          ...
          <div class="relative ">
            <div class="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
              <svg
                class="w-4 h-4 text-gray-500 dark:text-black-400"
                aria-hidden="true"
        ...
        <div class="relative mx-auto max-w-7xl">
          <div class="text-center">
            <h2 class="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
              From Classroom to Code,Idea's that Matter...
            </h2>
